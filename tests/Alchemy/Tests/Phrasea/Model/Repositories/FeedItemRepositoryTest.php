...
class FeedItemRepositoryTest extends \PhraseanetTestCase
{
    public function testIs_record_in_public_feedInPublicFeed()
    {
        ...
        $this->assertTrue(self::$DI['app']['EM']->getRepository('Phraseanet:FeedItem')->isRecordInPublicFeed($record->get_sbas_id(), $record->get_record_id()));
    }

    public function testIs_record_in_public_feedInPrivateFeed()
    {
        $record = self::$DI['record_2'];
        $this->assertFalse(self::$DI['app']['EM']->getRepository('Phraseanet:FeedItem')->isRecordInPublicFeed($record->get_sbas_id(), $record->get_record_id()));
    }
